...
.controller("recruitmentsCtrl", function ($scope, $http, $rootScope, $window, $cookieStore, $location , $routeParams) {
    ...
    $scope.exportExcel = function () {
        // $('#tdfromCountry').html($('#reqdate').val());
        // $('#tdfromCity').html($('#reqtime').val());

        // $('#tdCountry').html($('#selCarType').val());
        // $('#tdCity').html($('#txtMoreInfo').val());
        // $('#tdcost').html($('#selCountryF').val());
        // $('#tdcarType').html($('#selCityFrom').val());

        // $scope.checkOptions = [];
        // var sThisVal = [];

        // $('#selcarOption input:checked').each(function () {
        //     sThisVal.push($(this).val());
        // });
        // if (sThisVal.length > 0) {
        //     for (var i = 0; i < sThisVal.length; i++) {
        //         $('#tdCaroption').html($('#tdCaroption').val + "," + sThisVal[i]);
        //     }
        // }


        /*  var blob = new Blob([document.getElementById('requestdiv').innerHTML], {
...
.controller("editCandidateCtrl", function ($scope, $http, $rootScope, $window, $cookieStore, $location , $routeParams) {
    ...
    .then(function (response) {
        ...
        // $("input[name='gender']:checked").val($scope.data.gender);
        // $("input[name='specialNeeds']:checked").val($scope.data.specialNeeds);
        // $("input[name='scientificDegree']:checked").val($scope.data.scientificDegree);
